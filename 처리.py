...
def 매출메시지처리(토큰, 지출설정, 자금설정, 본문):
    ...
    # 1) 기존 계약 입금: 입금액이 계약금액과 '정확히' 맞고 이름/거래처도 맞는 행을 찾는다.
    #    - 클라이언트를 슬래시로 명시했으면 → 클라이언트 일치를 본다(흔한 단어 오매칭 방지).
    #    - 미명시면 → 계약명/클라이언트에 의미있는 토큰이 겹치는지(흔한 단어는 제외).
    ...
    for 행 in 데이터[1:]:
        ...
        if not 계약명 or 계약금액 != 입금액:     # 금액 정확 일치는 필수
            continue
        if 클라이언트:
            행클공백 = 클.replace(" ", "")
            맞음 = bool(행클공백) and (클공백 in 행클공백 or 행클공백 in 클공백)
        else:
            텍스트 = 계약명 + " " + 클
            맞음 = any(t in 텍스트 for t in 토큰들) if 토큰들 else False
        if 맞음:
            기존.append({"계약명": 계약명, "계약금액": 계약금액})

    if len(기존) == 1:
        c = 기존[0]
        응답 = 시트.매칭수정(
            자금설정["endpoint"], 자금설정["key"], "매출",
            {"계약명": c["계약명"], "계약금액": c["계약금액"]},
            {"입금일": 결과["날짜"], "입금액": 입금액, "입금상태": "입금완료"})
        if 응답.get("ok"):
            lark.메시지보내기(토큰, chat_id, "✅ 매출 입금 — %s\n   %s원 입금완료 · %s" % (
                c["계약명"], format(입금액, ","), 결과["날짜"][5:]))
        else:
            lark.메시지보내기(토큰, chat_id, "⚠️ '%s' 입금 기록 중 문제: %s" % (
                c["계약명"], 응답.get("error", "알 수 없음")))
        return True
