...
# ---------------- 매출(입금) ----------------
# 같은 지출방에 '입금'/'매출'이 들어간 글을 매출 칸의 해당 '계약'에 입금으로 채운다.
# 정석 = 기존 계약 행을 찾아 입금일·입금액·입금상태만 갱신(부가세·순매출 등 사람이 관리하는 칸은 안 건드림).
# 매칭은 이름(계약명/클라이언트) + 금액(계약금액)으로. 못 찾거나 여러 건이면 자동기록 대신 되묻는다.
def _금액(v):
    try:
        return int(float(v))
    except Exception:
...
def 매출메시지처리(토큰, 지출설정, 자금설정, 본문):
    ...
    토큰들 = [t for t in 힌트.split() if len(t) >= 2]
    후보 = []
    for 행 in 데이터[1:]:
        ...
        if not 계약명:
            continue
        텍스트 = 계약명 + " " + 클
        이름맞음 = any(t in 텍스트 for t in 토큰들) if 토큰들 else False
        금액맞음 = (계약금액 == 입금액)
        if 이름맞음 or (not 토큰들 and 금액맞음):
            후보.append({"계약명": 계약명, "계약금액": 계약금액, "금액맞음": 금액맞음})

    # 이름 후보가 여럿이면 금액 일치로 좁힌다(부가세 포함 총액 == 계약금액)
    if 토큰들:
        금액일치 = [c for c in 후보 if c["금액맞음"]]
        if 금액일치:
            후보 = 금액일치

    if len(후보) == 0:
        lark.메시지보내기(토큰, chat_id,
            "🔎 '%s'(%s원)에 맞는 계약을 매출 칸에서 못 찾았어요.\n"
            "   계약명을 더 정확히 적어주세요. 새 계약이면 알려주세요(아직 자동 추가는 안 합니다)."
            % (힌트 or "?", format(입금액, ",")))
        return True
    if len(후보) > 1:
        목록 = "\n".join("   · %s (%s원)" % (c["계약명"], format(c["계약금액"], ",")) for c in 후보[:6])
        lark.메시지보내기(토큰, chat_id,
            "🔎 '%s'에 맞는 계약이 여러 건이에요. 계약명을 더 정확히 적어주세요:\n%s" % (힌트, 목록))
        return True

    c = 후보[0]
    입금상태 = "입금완료" if 입금액 >= c["계약금액"] else "부분입금"
    미수금 = max(0, c["계약금액"] - 입금액)
    응답 = 시트.매칭수정(
        자금설정["endpoint"], 자금설정["key"], "매출",
        {"계약명": c["계약명"], "계약금액": c["계약금액"]},
        {"입금일": 결과["날짜"], "입금액": 입금액, "입금상태": 입금상태})
    if 응답.get("ok"):
        문구 = "✅ 매출 기록 — %s\n   입금 %s원 · %s · %s" % (
            c["계약명"], format(입금액, ","), 입금상태, 결과["날짜"][5:])
        if 입금상태 == "부분입금":
            문구 += "\n   (미수금 %s원 남음)" % format(미수금, ",")
        lark.메시지보내기(토큰, chat_id, 문구)
    else:
        lark.메시지보내기(토큰, chat_id,
            "⚠️ '%s' 입금 기록 중 문제: %s" % (c["계약명"], 응답.get("error", "알 수 없음")))
    return True
